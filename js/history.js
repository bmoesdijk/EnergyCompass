...
function hstGraphs(evt,btnName,dtdate,dspDate) {
            ...
            if (btnName == "Electricity" && document.getElementById("Setting_15").value != "DoNotShow" ) {
               ...
               success:function(data){
                        ...
                        var histMonthUsage = data.response18.split(",").map(Number).slice(1,-1);
                        var histMonthDelivery = data.response19.split(",").map(Number).slice(1,-1);
                        var histMonthMaxTemp = data.response20.split(",").map(Number).slice(1,-1);
                        var histDayHourlyTemp = data.response21.split(",").map(Number).slice(1,-1);
                       ...
                       switch (viewCase) {
                    	    case "01":
                            //Electricity with solar delivery graph{
                      		zingchart.render({
                        	id:"historyChart",
                        	width:"100%",
                        	height:400,
                        	  ...
                        	  data:{
                            	...
                            	"series":[{
                            		"type" : "bar", //Depending on gas or elec. this will be a bar or line chart
                                 	"line-color": graphColor,
                                 	"background-color": graphColor,
    								"values": histMonthUsage, 
                                 	"tooltip": {
                                    	 "text": tooltipTextDay
                            	 	  	}
                            		}, {
                                	"type": "bar",
      						  		"values": histMonthDelivery,
      						  		"line-color": delivGraphColor,
      						  		"tooltip": {
                                    	 "text": "Teruggeleverd %vt kW %kt",
                                    	 "font-color":labelColor
                        				...
                        				"type": "mixed",//Show the average temperature in the Gas monthChart
                            				...
                            				"scale-x": {
                                    		    ...
                                    		    "transform":{
                               				     ...
                               				     "items-overlap": true,
                                    		     "max-items":"25", //Show maximum 31 x-axis labels (days of the month)
                                    		     "item":{
                                          		      "visible":true
                                    		    }
                          			        },
						...
						case "04":
                    			//Gas without temperature graph      
                        			...
                        			data:{
     								...
     								break;
						    case "02":
     							 //Electricity without solar delivery graph
     								zingchart.render({
                        			id:"historyChart",
                        			width:"100%",
                        			height:400,
